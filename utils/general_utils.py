...
def colormap(img, cmap='jet'):
    ...
    # 关键修改：用 buffer_rgba() 替代 tostring_rgb()，并处理 alpha 通道
    # 1. 获取 RGBA 数据（包含 alpha 通道，格式为 [R, G, B, A]）
    rgba_data = np.frombuffer(fig.canvas.buffer_rgba(), dtype=np.uint8)
    # 2. 获取图像宽高（注意：get_width_height() 返回 (width, height)）
    width, height = fig.canvas.get_width_height()
    # 3. 重塑为 (height, width, 4)，取前 3 个通道（RGB），再展平为一维
    data = rgba_data.reshape((height, width, 4))[..., :3].flatten()
    # 4. 重塑为 (height, width, 3)（与原图尺寸匹配）
